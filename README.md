...
An agent midiAI starts runs as one of two **engines** -- **Claude Code** or
**Codex** -- and each engine reaches its model through one of four
**providers**: **Anthropic**, **OpenAI**, **AWS Bedrock**, **Google Vertex**.
`engines.py` holds it (`~/.midiai/providers.json`, `/providers`,
...
(`midiai-anthropic-api-key`), so a key saved before this file existed keeps
